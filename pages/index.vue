<template>
  ...
  <v-sheet class="mt-52 mx-auto p-10 border-primary-0 border-sm rounded-lg" elevation="2" width="600">
    <span>
      <div class="d-flex justify-center m-16">
        <svg class="h-11 w-56">
          <path d="M94.4536 32.7884V42.2693H54.0394V1.80615H66.2523V32.7884H94.4536Z" fill="var(--clr-body)"
          ...
          <path
            ...
            fill="var(--clr-body)" data-astro-cid-3ef6ksr2=""></path>
          <path
      ...
      <v-form ref="loginForm">
        <v-text-field v-model="login" label="Login" :rules="[loginRule]" variant="solo" rounded="lg"
          required></v-text-field>
        <v-text-field type="password" v-model="password" label="Password" :rules="[passwordRule]" variant="solo"
          rounded="lg" required></v-text-field>
        <v-btn class="mt-2 bg-secondary" type="button" block @click="checkInput()" rounded="lg"
          :loading="isloading"><span class="text-0 font-semibold text-base"> Connect </span></v-btn>
      </v-form>
    </span>
  </v-sheet>
</template>
