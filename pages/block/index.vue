<template>
  <v-row class="filter__quote">
    ...
    <v-col cols="2" class="flex items-center justify-end">
      <nuxt-link to="/block/create" class="w-full">
        <v-btn
            :disabled="loading"
            class="w-full"
            color="primary-0"
  ...
  <div v-else>
    ...
    >
      <template v-slot:item.image="{ item }">
        <nuxt-img v-if="item.images.length > 0" :src="item['images'][0]" class="w-24" />
      </template>
      <template v-slot:item.action="{ item }">
        <NuxtLink :to="`/block/${getIdFromIri(item['@id'])}/edit`">
...
watch(criteria, (data) => {
  if(data.name !== null && data.name !== "") {
    ...
    timer.value = setTimeout(() => {
      loading.value = true;
      getBlocks({page: data.page, itemsPerPage: data.itemsPerPage});
    }, 1000)
  }

}, {deep: true});

async function getBlocks({ page, itemsPerPage, sortBy }) {
  try {
    criteria.value.page = page;
    criteria.value.itemsPerPage = itemsPerPage;
    const objects = await $apinewsletter.getAllBlocks(criteria.value);
    blocks.value = objects["hydra:member"];
...
async function deleteIt(item)
{
  try{
    ...
    await getBlocks({page: 1, itemsPerPage: 20});
  }
  catch (error) {
    console.log(error);
    errorToast(error);
